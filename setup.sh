#!/usr/bin/env bash
# AI Bot - Linux/macOS Setup Script
...
# Step 2b: Check/Install Devin CLI (optional)
echo -e "${YELLOW}Checking Devin CLI (optional)...${NC}"
if command_exists devin; then
    echo -e "${GREEN}✓ Devin CLI is installed${NC}"
else
    echo -e "${YELLOW}Devin CLI not found. (Optional) Install with: curl -fsSL https://cli.devin.ai/install.sh | bash${NC}"
fi
...
# Optional: User ID for mentions when the AI finishes
# USER_ID=your_discord_user_id

# Optional: Category name for organizing channels
# CATEGORY_NAME=ai-bot

# Optional: Default AI provider (claude-code or devin)
...
if [ "$START_NOW" = "y" ] || [ "$START_NOW" = "Y" ]; then
    echo -e "${BLUE}Starting AI Bot...${NC}"
